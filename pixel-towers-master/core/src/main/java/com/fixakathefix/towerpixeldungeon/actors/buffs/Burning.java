...
public class Burning extends Buff implements Hero.Doom {
	...
	public boolean act() {
		...
		if (target.isAlive() && !target.isImmune(getClass())) {
			...
			int damage = Random.NormalIntRange(1, 4 + (int) Math.round(Math.sqrt(target.HT*0.1f)));//changed from  1, 3 + Dungeon.scalingDepth()/4, based on enemies' HT instead of depth
			Buff.detach( target, Chill.class);
