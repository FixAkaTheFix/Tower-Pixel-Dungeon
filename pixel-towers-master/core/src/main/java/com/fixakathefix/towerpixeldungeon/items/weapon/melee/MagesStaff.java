...
public class MagesStaff extends MeleeWeapon {
	...
	public int max(int lvl) {
		return  Math.round(8f*damageModifier() +   //6 base damage, down from 10
				lvl*damageModifier());               //scaling unaffected
	}

	@Override
	public int min(int lvl) {
		return  Math.round(3f*damageModifier() +   //6 base damage, down from 10
				lvl*damageModifier());
	}
	...
	public void updateWand(boolean levelled){
		if (wand != null) {
			...
			wand.maxCharges = Math.min(wand.maxCharges + 1, 30);
