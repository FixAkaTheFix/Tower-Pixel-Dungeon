...
public class TimekeepersHourglass extends Artifact {
	...
	public static final String AC_GIVETIME = "GIVETIME";
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if (isEquipped( hero )
			...
			if (level()<20) actions.add(AC_GIVETIME);
		}
		return actions;
	}
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals(AC_GIVETIME)){
			if (!isEquipped(hero)) GLog.i( Messages.get(Artifact.class, "need_to_equip") );
			else if (hero.buff(Slow.class)!=null) GLog.w( Messages.get(this, "no_time") );
			else {
				upgrade();
				chargeCap = 3+(int)(level()*1.5f);
				Buff.affect(hero, Slow.class, level()*8 + 5);
			}
		}
	}
		...
		if (isEquipped( Dungeon.hero )){
			if (!cursed) {
				if (level() < levelCap ){
					...
					desc += "\n\n" + Messages.get(this, "canupgrade");
				} else {
					desc += "\n\n" + Messages.get(this, "cannotupgrade");
				}
