...
import com.fixakathefix.towerpixeldungeon.sprites.RipperSprite;
...
public class RipperDemon extends Mob {

	{
		...
		targetingPreference = Random.oneOf(TargetingPreference.NOT_WALLS, TargetingPreference.NORMAL, TargetingPreference.NORMAL, TargetingPreference.NORMAL);
	}
			...
			if (leapPos != -1){
				...
				sprite.jump(pos,
					...
					public void call() {
						...
						if (leapVictim != null && alignment != leapVictim.alignment){//checks whether the victim is and ally, or if the demon misses
							if (hit(RipperDemon.this, leapVictim, Char.INFINITE_ACCURACY, DamageType.PHYSICAL)) {
						...
						if (endPos != leapPos){
							Actor.addDelayed(new Pushing(RipperDemon.this, leapPos, endPos), -1);//why the demon attacks instantly after the leap @FIX
						}

						pos = endPos;//finalpos of the demon = endPos
						leapPos = -1;
						sprite.idle();
