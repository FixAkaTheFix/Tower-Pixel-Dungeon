...
public class WandOfCorruption extends DamageWand {
	...
	public int min(int lvl){
		return Math.round((2+lvl)*magicDamageModifier());
	}
	...
	public void onZap(Ballistica bolt) {
		...
		if (ch != null){
			...
			//100% health: 5x resist   75%: 3.25x resist   50%: 2x resist   25%: 1.25x resist. Boss - full resist, miniboss - the resist is doubled.
			enemyResist *= 1 + 4*Math.pow(enemy.HP/(float)enemy.HT, 2);
			if (enemy.properties().contains(Char.Property.MINIBOSS)) enemyResist*=2;
			if (enemy.properties().contains(Char.Property.MINIBOSS)) enemyResist*=100;
