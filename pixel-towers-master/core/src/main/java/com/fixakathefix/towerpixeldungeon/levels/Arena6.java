...
public class Arena6 extends ArenaPrison {
    /**
     * A map filled with guards, thieves, with a lot of dialogue and captain surviving side-quest.
     * introduction to the guards, encourages trap usage.
     */
    {
        name = "Prison";
        ...
        for (int y = 1; y <= 93; y += 7) {
            ...
            for (int x = 1; x <= 49; ) {
                ...
                switch (type) {
                    ...
                    case 12: {//gmimic cell
                        Painter.fill(this, x + 1, y + 1, 5, 5, Terrain.EMPTY);
                        this.mobs.add(GoldenMimic.spawnAt(Random.element(candidates), Generator.random(Generator.Category.ARTIFACT)));
                        this.map[exitpos] = Terrain.LOCKED_DOOR;
                        break;
                    }
                    case 13: {//chest cell
    ...
    public TowerGuard3 captain = new TowerGuard3();
    ...
    public void initNpcs() {
        ...
        for (int i : guardposes) {
            if (Math.random()>0.2){
                ...
                guard.pos = i;
                GameScene.add(guard);
                guard.defendPos(guard.pos);
            } else {
                SleepyThief thif = new SleepyThief();
                thif.pos = i;
                GameScene.add(thif);
            }
        }
    ...
    public class TowerShopKeeperKeytrader extends TowerShopKeeper {
        ...
        public TowerShopKeeperKeytrader(){
            super();
        }
    ...
    public static class SleepyThief extends Bandit{
        {
            state = SLEEPING;
        }
