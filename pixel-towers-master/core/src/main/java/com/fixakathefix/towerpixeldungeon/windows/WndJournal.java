...
import com.fixakathefix.towerpixeldungeon.actors.mobs.Pylon;
...
import com.fixakathefix.towerpixeldungeon.ui.QuickRecipe;
...
import com.fixakathefix.towerpixeldungeon.ui.ScrollingGridPane;
import com.fixakathefix.towerpixeldungeon.ui.ScrollingListPane;
...
public class WndJournal extends WndTabbed {
	...
	public static class CatalogTab extends Component{
		...
		protected void createChildren() {
			...
			itemButtons[EQUIP_IDX].icon(new ItemSprite(ItemSpriteSheet.BATTLE_AXE));
			itemButtons[CONSUM_IDX].icon(new ItemSprite(ItemSpriteSheet.POTION_FROST));
	...
	private static void addGridItems( ScrollingGridPane grid, Collection<Class<?>> classes) {
		for (Class<?> itemClass : classes) {
			...
			if (Item.class.isAssignableFrom(itemClass)) {
				...
				if (seen){
					sprite = new ItemSprite(ItemSpriteSheet.SWORD, ench.glowing());
					title = Messages.titleCase(ench.name());
					desc = ench.desc();
				} else {
					sprite = new ItemSprite(ItemSpriteSheet.SWORD);
					sprite.lightness(0f);
					title = "???";
			...
			if (Mob.class.isAssignableFrom(entityCls)) {
				...
				if (seen) {
					...
					if (Bestiary.encounterCount(entityCls) > 1){
						if (Bestiary.ALLY.entities().contains(entityCls) || Bestiary.TOWERS.entities().contains(entityCls))
						desc += "\n\n" + Messages.get(CatalogTab.class, "friend_count", Bestiary.encounterCount(entityCls));
