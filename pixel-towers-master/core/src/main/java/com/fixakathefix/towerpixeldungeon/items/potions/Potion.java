...
import com.fixakathefix.towerpixeldungeon.items.potions.exotic.PotionOfSnapFreeze;
...
public class Potion extends Item {
	...
	private static final LinkedHashMap<String, Integer> colors = new LinkedHashMap<String, Integer>() {
		{
			put("crimson",ItemSpriteSheet.POTION_STRENGTH);
			put("amber",ItemSpriteSheet.POTION_HEAL);
			put("golden",ItemSpriteSheet.POTION_MINDVISION);
			put("jade",ItemSpriteSheet.POTION_FROST);
			put("turquoise",ItemSpriteSheet.POTION_FLAME);
			put("azure",ItemSpriteSheet.POTION_TOXIGGAS);
			put("indigo",ItemSpriteSheet.POTION_SPEED);
			put("magenta",ItemSpriteSheet.POTION_INVISIBILITY);
			put("bistre",ItemSpriteSheet.POTION_LEVITAION);
			put("charcoal",ItemSpriteSheet.POTION_PARALYTICGAS);
			put("silver",ItemSpriteSheet.POTION_CLEANSING);
			put("ivory",ItemSpriteSheet.POTION_BLESSING);
		}
	};
			...
			GameScene.show(
				new WndOptions(new ItemSprite(this),
					...
					protected void onSelect(int index) {
						if (index == 0) {
							Statistics.potions++;
							if (Statistics.potions >= 10) Badges.validatePotionAddict();
