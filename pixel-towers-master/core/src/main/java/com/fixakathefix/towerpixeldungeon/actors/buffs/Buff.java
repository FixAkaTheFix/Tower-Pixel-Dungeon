...
public class Buff extends Actor {
	...
	public boolean attachTo( Char target ) {
		if (DeviceCompat.isDebug()) DeviceCompat.log( "BUFF",
				name()+
						" attached to "+
						target.name());
	...
	public static<T extends FlavourBuff> T affect( Char target, Class<T> buffClass, float duration ) {
		if (target instanceof BossImage){
			BossImage mob = (BossImage) target;
			affect(mob.assignedMob, buffClass, duration);//images transfer their buffs

			T bu = affect( mob.assignedMob, buffClass );
			bu.spend(duration*mob.assignedMob.resist(buffClass));
			return bu;
		}
	...
	//depletes the time of the buff
	public static<T extends FlavourBuff> T shorten( Char target, Class<T> buffClass, float duration ) {
		T buff = affect( target, buffClass );
		buff.setTime(buff.getTime() - duration);
		return buff;
	}
