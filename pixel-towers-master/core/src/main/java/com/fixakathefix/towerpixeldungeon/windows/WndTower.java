...
public class WndTower extends WndInfoMob {
    ...
    public WndTower(Tower tower) {
        ...
        int cost = (tower instanceof TowerTntLog) ? Math.round(tower.cost * 0.3f)
                : Math.round(tower.cost * 0.3f + tower.cost * 0.5f *((float)Math.min(tower.HP, tower.HT)/tower.HT));

        if (Dungeon.hero.isAlive()) {
            {
                y += GAP;
                ArrayList<RedButton> buttons = new ArrayList<>();
                RedButton sellbtn = new RedButton(Messages.get(this, "sellbutton", cost), 8, Chrome.Type.YELLOW_BUTTON) {
                    @Override
                    ...
                    protected void onClick() {
                        ...
                        GameScene.selectCell(new CellSelector.Listener() {
                            ...
                            public void onSelect(Integer cell) {
                                if (cell == null) return;
                                ((SentientTower) tower).directTocell(cell);
                            }
