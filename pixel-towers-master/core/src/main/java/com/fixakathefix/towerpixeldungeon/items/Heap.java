...
public class Heap implements Bundlable {
	...
	public void drop( Item item ) {
		...
		//Lost backpack, and selling items are below the heaps to prevent bugs including those replacing a selling heap if dead on a selling place
		if (item.dropsDownHeap && type == Type.FOR_SALE && (peek() instanceof LostBackpack || peek() instanceof Dewdrop)) {
			items.addLast( item );
		} else {
			items.add( item );
		}
