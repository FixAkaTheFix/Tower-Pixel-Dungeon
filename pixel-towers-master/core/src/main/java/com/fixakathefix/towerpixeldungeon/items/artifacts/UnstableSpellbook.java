...
public class UnstableSpellbook extends Artifact {

	{
		...
		charge = (int)((level()*0.2f)+1);
		partialCharge = 0;
		chargeCap = (int)((level()*0.2f)+1);
	 ...
	 * 1. Accepts 5 scrolls at once.
	 * 2. Does not roll op battle scrolls
	 *
	 */
			...
			if (hero.buff( Blindness.class ) != null) GLog.w( Messages.get(this, "blinded") );
				...
				do {
					scroll = (Scroll) Generator.randomUsingDefaults(Generator.Category.SCROLL);
						...
						//reduce the frequency of these scrolls by x3
						||((scroll instanceof ScrollOfSkulls ||
						scroll instanceof ScrollOfTerror ||
						scroll instanceof ScrollOfUpgrade ||
							scroll instanceof ScrollOfDrowsiness) && Random.Int(3) == 0));
				curItem = scroll;
				curUser = hero;
	...
	public String desc() {
		...
		if (isEquipped(Dungeon.hero)) {
			...
			if (level() < levelCap && scrolls.size() > 0) {
				...
				if (scrolls.size() > 2)
					desc += "\n" + "_" + Messages.get(scrolls.get(2), "name") + "_";
				if (scrolls.size() > 3)
					desc += "\n" + "_" + Messages.get(scrolls.get(3), "name") + "_";
				if (scrolls.size() > 4)
					desc += "\n" + "_" + Messages.get(scrolls.get(4), "name") + "_";
			}
		}
	...
	public class bookRecharge extends ArtifactBuff{
		...
		public boolean act() {
			if (charge < chargeCap
				...
				//200 to 66 turns
				float chargeGain = 0.005f + (float)0.001*level();
	...
	protected WndBag.ItemSelector itemSelector = new WndBag.ItemSelector() {
		...
		public void onSelect(Item item) {
			if (item != null && item instanceof Scroll && item.isIdentified()){
				...
				for (int i = 0; ( i <= 4 && i < scrolls.size() ); i++){
