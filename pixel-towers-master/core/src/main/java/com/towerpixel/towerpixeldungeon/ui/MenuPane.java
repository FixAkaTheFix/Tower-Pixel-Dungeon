...
import com.towerpixel.towerpixeldungeon.windows.WndChallenges;
import com.towerpixel.towerpixeldungeon.windows.WndGame;
import com.towerpixel.towerpixeldungeon.windows.WndJournal;
...
public class MenuPane extends Component {
	...
	protected void createChildren() {
		...
		String arenaname = "arena" + Dungeon.depth + "name";

		depthName = new BitmapText(Messages.get(this , arenaname),PixelScene.pixelFont);
		depthName.hardlight(0xCACFC2);
		depthName.measure();
		add(depthName);
	...
	protected void layout() {
		...
		depthIcon.x = btnJournal.left() - 10 + (7 - depthIcon.width())/2f - 0.1f;//TODO CHANGE THIS: THE WAVE COUNTER
		depthIcon.y = y + 1;
		if (SPDSettings.interfaceSize() == 0) depthIcon.y++;
		PixelScene.align(depthIcon);

		depthText.scale.set(PixelScene.align(0.67f));
		depthText.x = depthIcon.x + (depthIcon.width() - depthText.width())/1.9f;
		depthText.y = depthIcon.y + depthIcon.height();
		PixelScene.align(depthText);

		depthName.scale.set(PixelScene.align(0.67f)/2f);
		depthName.x = depthIcon.x - depthName.width()*1.05f;
		depthName.y = depthIcon.y + depthIcon.height*0.25f;
		PixelScene.align(depthName);
