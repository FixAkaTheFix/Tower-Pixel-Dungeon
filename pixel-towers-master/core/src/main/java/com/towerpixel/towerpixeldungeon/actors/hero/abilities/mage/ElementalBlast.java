...
package com.towerpixel.towerpixeldungeon.actors.hero.abilities.mage;
...
public class ElementalBlast extends ArmorAbility {
	...
	static {
		...
		effectTypes.put(WandOfSnakes.class,         MagicMissile.PURPLE_CONE);
	...
	static {
		...
		damageFactors.put(WandOfSnakes.class,   0.5f);
					...
					public void call() {
						...
						for (int cell : aoe.cells) {
							...
							if (mob != null && mob != hero){
								...
								//*** Wand Of Snakes ***
								} else if (finalWandCls == WandOfSnakes.class){
									if (mob.isAlive() && mob.alignment != Char.Alignment.ALLY) {
										Buff.affect( mob, Poison.class ).set(10*effectMulti);
									}
