	...
	public void doRead() {
		...
		for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
			if (Dungeon.level.heroFOV[mob.pos]) {
				...
				if (!(mob.alignment == Char.Alignment.ALLY || mob.properties().contains(Char.Property.BOSS) || mob.properties().contains(Char.Property.MINIBOSS)))
					mob.damage(Math.round(mob.HT/10f + (mob.HP * power * 0.1f)), this);
				if (mob.isAlive()) {
