...
package com.towerpixel.towerpixeldungeon.effects;
...
public class Fireball extends Component {
	...
	private final float basex = x;
	private final float basey = y;
	...
	protected void createChildren() {
		...
		bLight = new Image( Assets.Interfaces.FIREBALL );
		bLight.frame( BLIGHT );
		...
		fLight = new Image( Assets.Interfaces.FIREBALL );
		fLight.frame( FLIGHT );
	...
	public void update() {
		...
		if (Random.Float() < Game.elapsed) {
			...
			spark.reset( x, y, ColorMath.random( 0xFF3311, 0xFFFF66 ), 2, Random.Float( 0.5f, 1.0f ) );
			spark.speed.set(
				Random.Float( -40, +40 ),
				Random.Float( -60, +20 ) );
			spark.acc.set( 0, +80 );
	...
	public static class Flame extends Image {
		...
		public Flame() {
			...
			super( Assets.Interfaces.FIREBALL );
