...
public class Drill extends Mob {

    {
        spriteClass = DrillSprite.class;
        state = WANDERING;
        HP = HT = 880055535;
        defenseSkill = 0;
    ...
    public void moveForward() {
        ...
        for (int x = v % Dungeon.level.width()+3; x>v % Dungeon.level.width()-1 ;x--) for (int y = v / Dungeon.level.width()+2; y>v / Dungeon.level.width()-1 ;y--){
    ...
    protected boolean act() {
        ...
        int[] ints = new int[]{
                ...
                pos+5+Dungeon.level.width()+Dungeon.level.width(),
                pos+4+Dungeon.level.width()+Dungeon.level.width(),
                pos+5+Dungeon.level.width(),
                pos+4+Dungeon.level.width(),
                pos+5,
                pos+4,
        };
        for (int i : ints) {
            if (Char.findChar(i)!= null){
                ...
                if (Char.findChar(i)!= null) WandOfBlastWave.throwChar(Char.findChar(i), new Ballistica(i, i+6, Ballistica.PROJECTILE), 6,false,true, Drill.class);
        ...
        int[] ints2 = new int[]{
                ...
                pos+5,
                pos+4,
                pos+4+Dungeon.level.width()+Dungeon.level.width()+Dungeon.level.width(),
                pos+4-Dungeon.level.width()
        };
        for (int i : ints2) {
